...
require_once JKit::find_file('vendor', 'OAuth/Client');
/**
 * 豆瓣API SDK
...
class Model_Douban extends OAuth_Interface
{
    ...
    public static function getAccessToken()
    {
        ...
        parse_str($response,$rt);
        if( isset( $rt['oauth_token'] ) && $rt['oauth_token'] && $rt['oauth_token_secret'] )
        {
            self::getOAuth()->setTokenSecret($rt['oauth_token_secret']);
        ...
        JKit::$log->warn(__FUNCTION__." url-".self::$accessTokenURL.",oauth_token-".
        	self::getParam(self::OAUTH_TOKEN).",ret-". $response);
        return false;
    }
     ...
     */
    public static function call($command , $params=array() , $method = 'GET' , $extheaders=array(), $multi=false ,$decode=true , $format=self::RETURN_JSON)
    {
        ...
        $response = self::request( 'http://api.douban.com/'.ltrim($command,'/') , $method, $params, $multi, $extheaders);
        JKit::$log->debug(__FUNCTION__." url-"."http://api.douban.com/".ltrim($command,'/').", ret-". $response);
        if($decode)
        {
    ...
    protected static function getOAuth()
    {
        if( null === self::$oauth )
        {
            self::$oauth = new OAuth_Client(self::$_appsecret,self::$_debug);
     ...
     * @ignore
     */
    protected static function request($url , $method , $params , $multi=false, $extheaders=array())
    {
...
//        print_r($params);
    ...
    public static function getHttpCode() {
    	return self::getOAuth()->getHttpCode();
    }
}
