...
class Model_Qq extends OAuth_Interface
{
     ...
     */
    const ACCESS_TOKEN = 'qq_access_token';

    /**
     * 存储refresh_token的session key
     */
    const REFRESH_TOKEN = 'qq_refresh_token';

    /**
     * 存储expires_in的sieesion key
     */
    const EXPIRES_IN = 'qq_expires_in';

    /**
     * 存储scope 的session key
     */
    const SCOPE = 'qq_scope';

    /**
     * 存储OPENID的session key
     */
    const OPENID = 'qq_openid';
...
//            self::setParam(self::SCOPE, $token['scope']);
        ...
        else
        {
            ...
            JKit::$log->warn(__FUNCTION__."get access token failed, ret-{$response}, param-",$params);
            return false;
    ...
    protected static function getOpenID()
    {
        ...
        $token = OpenSDKUtil::json_decode($json, true);
        if($token['openid']) {
            self::setParam(self::OPENID, $token['openid']);
        } else {
        	JKit::$log->warn( __FUNCTION__." ret-".$json );
        }
